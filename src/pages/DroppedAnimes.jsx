...
const DroppedAnimes = () => {
    ...
    const { value: newEpisode } = await Swal.fire({
      ...
      html: `
        <div style="display:flex;flex-direction:column;gap:14px;text-align:left;margin-top:10px;">
          <p style="color:#888;font-size:0.85rem;margin:0;">Estás editando: <strong style="color:#fff;">${anime.titulo}</strong></p>
          <label style="color:#888;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Episodio en el que lo dejaste</label>
          <input 
            id="swal-ep-quick" 
            type="number" 
            min="0" 
            ...
            style="margin:0;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(62,166,255,0.25);border-radius:8px;"
          >
          ${anime.episodios_totales ? `<span style="color:#555;font-size:0.75rem;">Límite de episodios: ${anime.episodios_totales}</span>` : ''}
        </div>`,
      focusConfirm: false,
  ...
  const verMotivo = (anime) => {
    Swal.fire({
      title: anime.titulo,
      html: `
        <p style="color:#555;font-size:0.72rem;margin:0 0 12px;text-transform:uppercase;letter-spacing:1.4px;font-weight:700;">
          Por qué lo dropeaste
        </p>
        <p style="color:#ccc;font-size:0.96rem;line-height:1.75;font-style:italic;margin:0;padding:14px 16px;background:rgba(255,255,255,0.04);border-radius:10px;border-left:3px solid #3ea6ff;">
  ...
  const handleEdit = async (anime) => {
    const { value: formValues } = await Swal.fire({
      ...
      html: `
        <div style="display:flex;flex-direction:column;gap:14px;text-align:left;margin-top:10px;">
          <label style="color:#888;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Episodios vistos</label>
          <input 
            id="swal-ep" 
            type="number" 
            min="0" 
            ...
            style="margin:0;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(62,166,255,0.25);border-radius:8px;"
          >
          ${anime.episodios_totales ? `<span style="color:#555;font-size:0.75rem;margin-top:-4px;">Límite de episodios: ${anime.episodios_totales}</span>` : ''}
          
          <label style="color:#888;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:1px;">Motivo del dropeo</label>
          <textarea id="swal-note" class="swal2-textarea"
            style="margin:0;background:rgba(255,255,255,0.05);color:#fff;border:1px solid rgba(62,166,255,0.25);border-radius:8px;resize:vertical;min-height:90px;"
  ...
  return (
    <div>
      <style>{`
        ...
        .dg-search-input::placeholder { color: #3a3a3a; }
        ...
        .dg-card:hover {
          ...
          box-shadow: 0 20px 55px rgba(0,0,0,0.7), 0 0 0 1px rgba(62,166,255,0.08);
        }
        ...
        .dg-note-badge.has-note { color: rgba(62,166,255,0.55); }
        .dg-note-badge.no-note  { color: #333; }
        .dg-dot {
          width: 5px; height: 5px;
          border-radius: 50%;
          flex-shrink: 0;
        }
        .has-note .dg-dot { background: #3ea6ff; opacity: 0.6; }
        .no-note  .dg-dot { background: #2e2e2e; }
        ...
        .dg-loading p, .dg-empty p { font-size:0.88rem; color:#333; margin:0; }
      ...
      {loading ? (
        ...
        <div className="dg-empty">
          <Skull size={46} color="rgba(255,255,255,0.05)" />
        ...
        <div className="dg-grid">
          {filtered.length === 0 ? (
            <div className="dg-no-results">
              ...
              <p>Sin resultados para "<strong style={{color:'#444'}}>{query}</strong>"</p>
